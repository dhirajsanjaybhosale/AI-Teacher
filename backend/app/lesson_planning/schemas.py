...
class Segment(BaseModel):
    ...
    whiteboard_data: Optional[Dict[str, Any]] = Field(default=None, description="Dynamic structured smart whiteboard data (code/execution/output, equation/steps/graph, diagram/formula/calc, etc.)")
    ...
    audio_url: Optional[str] = Field(default=None, description="URL or relative path to the generated segment audio")
    target_seconds: Optional[int] = Field(default=120, description="Target duration allocated for this segment in seconds")
    actual_seconds: Optional[float] = Field(default=0.0, description="Actual measured audio duration in seconds")
    thinking_seconds: Optional[int] = Field(default=20, description="Thinking pause duration for interactive check in seconds")
...
class LessonPlan(BaseModel):
    ...
    prerequisites: Optional[List[str]] = Field(default_factory=list, description="Foundational prerequisites for this lesson")
    target_level: str = Field(default="beginner", description="beginner, intermediate, or advanced")
    target_language: str = Field(default="en", description="Language code: en (English), hi (Hindi), mr (Marathi), or hinglish")
    estimated_minutes: int = Field(default=10, description="Total time budget in minutes")
    target_duration_seconds: Optional[int] = Field(default=600, description="Target duration in seconds")
    actual_duration_seconds: Optional[float] = Field(default=0.0, description="Measured actual audio duration in seconds")
    video_duration_seconds: Optional[float] = Field(default=0.0, description="Measured actual video duration in seconds")
    full_video_url: Optional[str] = Field(default=None, description="URL or relative path to the assembled full lesson video")
    manifest: Optional[Dict[str, Any]] = Field(default=None, description="Video segment manifest with media durations")
    grounded_source_display: Optional[str] = Field(default="", description="Student friendly notice: Grounded in DBMS.pdf")
